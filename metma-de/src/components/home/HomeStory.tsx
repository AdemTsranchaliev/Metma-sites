    ...
    <section className="bg-[var(--metma-sand)] py-14 sm:py-16 md:py-28">
      <div className="container-metma grid gap-8 lg:grid-cols-[1fr_1.15fr] lg:items-start lg:gap-16">
        ...
        <div className="flex flex-col gap-2.5 sm:gap-3">
          {points.map((point, i) => (
            <Reveal key={point.n} delayMs={i * 80}>
              <Link
                href={point.href}
                className="grid grid-cols-[auto_1fr] gap-3.5 px-4 py-5 transition active:scale-[0.99] sm:gap-7 sm:px-7 sm:py-7 sm:hover:-translate-y-0.5"
                style={{ background: point.bg }}
              >
                <span
                  className="font-display text-2xl font-bold leading-none tracking-tight sm:text-4xl"
                  style={{ color: point.num }}
                >
                  {point.n}
                </span>
                <div>
                  ...
                  <p className="mt-1.5 text-sm leading-6 text-[var(--metma-mute)] sm:mt-2 sm:leading-7">
                    {point.text}
                  </p>
                </div>
              </Link>
